...
int findNumItems(char* filename) {
    ...
    if (fp == NULL) {
        printf("Inventory file DOES NOT EXIST! Exiting ...");
        exit(1);
    }
    char chr = getc(fp);
    int itemNumber = 0;
...
void readPreviousOutputFile(char* filename, item** itemArray) {
    ...
    FILE * fp = fopen(filename, "r");
    if (fp == NULL) {
        printf("Could not open previous output file\n");
        exit(0);
    }
...
void modeModify(item** itemArray, int itemNumber) {
    ...
    while (itemIndex < itemNumber+1) {
        ...
        if (check == -1) {
            printf("The item you are trying to modify does not currently exist\n");
        }
        itemIndex++;
    }
}
